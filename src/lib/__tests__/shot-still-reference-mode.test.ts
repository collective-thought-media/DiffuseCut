...
describe("shot-still-reference-mode", () => {
  ...
  it("routes integrate in scene to the masked inpaint template", () => {
    ...
    expect(plan.generationOptions.locationPlateDenoise).toBe(1.0);
    expect(plan.generationOptions.integrateSubjectHeightFraction).toBe(0.4);
    ...
    // Style-transfer character IP was only tinting plates; Integrate uses
    // linear IP so a real figure forms in the subject mask.
    expect(plan.generationOptions.ipAdapterWeight).toBe(0.78);
    expect(plan.generationOptions.ipAdapterEndAt).toBe(0.85);
    expect(plan.generationOptions.ipAdapterWeightType).toBe("linear");
