import type { NextRequest } from "next/server";
import {
  jsonOk,
  jsonError,
  handleApiError,
  parseJson,
} from "@/lib/api-helpers";
import { installShotRenderVideo } from "@/lib/services/install-shot-video";
import {
  fetchUrlToBuffer,
  MAX_UPLOAD_BYTES,
} from "@/lib/services/media-import";

type RouteParams = { params: Promise<{ id: string; shotId: string }> };

export async function POST(req: NextRequest, { params }: RouteParams) {
  try {
    const { id: projectId, shotId } = await params;
    const contentType = req.headers.get("content-type") ?? "";

    if (contentType.includes("application/json")) {
      const body = await parseJson<{ url?: string }>(req);
      if (!body.url?.trim()) return jsonError("url is required", 400);
      let parsedUrl: URL;
      try {
        parsedUrl = new URL(body.url);
      } catch {
        return jsonError("url must be a valid URL", 400);
      }
      if (!["http:", "https:"].includes(parsedUrl.protocol)) {
        return jsonError("url must use http or https", 400);
      }
      const { buffer, fileName } = await fetchUrlToBuffer(body.url);
      const result = await installShotRenderVideo({
        projectId,
        shotId,
        buffer,
        fileName,
      });
      return jsonOk(result, 201);
    }

    const formData = await req.formData();
    const file = formData.get("file");
    if (!(file instanceof File)) {
      return jsonError("file is required", 400);
    }
    if (file.size > MAX_UPLOAD_BYTES) {
      return jsonError(`File exceeds ${MAX_UPLOAD_BYTES} byte limit`, 413);
    }
    const buffer = Buffer.from(await file.arrayBuffer());
    const result = await installShotRenderVideo({
      projectId,
      shotId,
      buffer,
      fileName: file.name || "clip.mp4",
    });
    return jsonOk(result, 201);
  } catch (err) {
    if (err instanceof Error && err.message.endsWith("not found")) {
      return jsonError(err.message, 404);
    }
    if (
      err instanceof Error &&
      err.message === "Install clip needs a video file."
    ) {
      return jsonError(err.message, 400);
    }
    return handleApiError(err);
  }
}
