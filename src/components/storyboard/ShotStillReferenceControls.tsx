...
const MODE_LABELS: Record<ShotStillReferenceMode, string> = {
  ...
  integrate_in_scene:
    "Integrate in scene: paint into the location plate, then diffuse the edges",
  composited:
    ...
    <div className="space-y-3 rounded-lg border border-neutral-800 bg-neutral-950 p-3">
      ...
      {showIdentityStrength && onIdentityStrengthChange ? (
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-muted-foreground">
            ...
            scene it bumps likeness without crushing pose. Lower it when the
            prompt should lead pose and framing.
          </p>
        </div>
      ) : null}
      ...
      {mode === "integrate_in_scene" ? (
        <p className="text-xs text-muted-foreground">
          ...
          plate, then runs a second diffusion pass on the subject edges so
          lighting and contact match the set. The rest of the plate stays
          pixel-locked. Subject size controls how large the character renders.
          This is not a hard cutout paste.
        </p>
      ) : null}
      {mode === "dual" ? (
        <p className="text-xs text-muted-foreground">
